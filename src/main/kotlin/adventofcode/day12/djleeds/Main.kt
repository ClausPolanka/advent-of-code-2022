...
data class Coordinates(val x: Int, val y: Int) {
    val adjacent: List<Coordinates> by lazy {
        listOf(Coordinates(x - 1, y), Coordinates(x + 1, y),
...
class Terrain(private val plots: List<List<Plot>>) {
    ...
    fun shortestDistance(isDestination: (Plot) -> Boolean): Int {
        ...
        val steps: Queue<Step> =
            LinkedList<Step>().apply { add(Step(endPoint)) }
        ...
        throw IllegalStateException(
            "Could not find a path that meets the criteria.")
    }

    private fun findPlot(predicate: (Plot) -> Boolean) =
        plots.flatten().first(predicate)

    private operator fun <T> List<List<T>>.get(coordinates: Coordinates) =
        this[coordinates.y][coordinates.x]

    private class Bounds(private val eastWestBounds: IntRange,
                         private val northSouthBounds: IntRange) {
...
fun main() {
    ...
    val testInput = readInput("day12/sample")
    println("fewest steps required to move from your current position to the " +
            "location : " + part1(testInput))
